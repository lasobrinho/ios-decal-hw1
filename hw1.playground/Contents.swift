...
//: Because let is a keyword for a constant value, and inside the for loop 'i' cannot be a constant because its value is changing every iteration, so it should be a var instead of a let declaration. It's missing a 'return true' statement at the very end of the function.
...
//: Before the fix, 'countLetter' wasn't initialized, that was only declared. 'lenA' and 'lenB' should be let, not var. 'letter' replaced by underscore because 'letter' var is never used. 'return true' statement at the very end because it can't return 'nil'.
